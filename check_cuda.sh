...
# NVIDIA's pip wheels are not automatically on the dynamic-linker search path.
# Keep this discovery relative to the project so it also works after Python is
# upgraded from 3.14 to another version.
shopt -s nullglob
library_dirs=()
...
exec "$PYTHON_BIN" "$ROOT_DIR/check_cuda.py" "$@"
